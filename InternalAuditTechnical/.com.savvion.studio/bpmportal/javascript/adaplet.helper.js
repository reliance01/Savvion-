//sbm.adaplet.invoke = function invoke() {};
sbm.adaplet.renderChart = function(p_type ,pt_name, adapletName, chart_type, div_name, caption, Xlabel, Ylabel, width, height)
{
    ...
    chartMap = {'chartType':chart_type,'width':width,'height':height,'caption':caption,'subcaption':'Powered by Savvion Business Manager','xAxisName':Xlabel,'yAxisName':Ylabel,'numberPrefix':''};
...
sbm.adaplet.renderTable = function (id,list) {
     // render the 2d list returned from server as an html table
     if($(id) == undefined)
          alert("div with '"+id+"' no found!");
     var tableHeader = "";
     var tableBody = "";
     
     for(var i=0;i < list[0].length;i++){
            tableHeader += "<th>"+ list[0][i] +"</th>";
     }    
     ...
     var table       = "<table class=\"sortable\" id=\"table-"+id+"\" width=\"100%\" border=\"0\" padding=\"0\">"+
                       "    <thead>"+
		       "          <tr>"+ tableHeader + "</tr>"+
		       "    </thead>"+
...
var image_path = "/sbm/bpmportal/javascript/images/";
