...
function init(datetimeString)
{
	...
	var dateInLong = (((datetimeString == '') || (datetimeString.indexOf('AM') != -1) || (datetimeString.indexOf('PM') != -1))?date2Long(datetimeString):parseInt(datetimeString));
	if (eval(debug))
	{
...
function date2Long(calendarString)
{
	...
	if (!((calendarString == '') || (calendarString.indexOf('AM') != -1) || (calendarString.indexOf('PM') != -1)))
		return;
...
function long2Date(datetime)
{
	...
	if (isNaN(parseInt(datetime.value)))
		return;
	datetime.value = convertLong2Date(datetime.value);
	if (eval(debug))
	{
...
function getDateAndTime(date)
{
	...
	if (hour == 12 && ampm == 'AM')
		hour = 0;
	if (hour < 12 && ampm == 'PM')
		hour = new Number(hour) + new Number(12);
