	...
	var count = 0;
		for (var i=0;i < srcDsArray.length;i++) {
			...
			if (value == srcRec.type) {
				addRow('srcTable',i,count,'srcRadioButton',srcRec.type,srcRec.name);
				count++;
			} else if (value == "-1") {
				addRow('srcTable',i,count,'srcRadioButton',srcRec.type,srcRec.name);
				count++;
	...
	for (var i=0;i < tgtDsArray.length;i++) {
		...
		if (found == false) {
			if (value == tgtRec.type) {
				addRow('tgtTable',i,count,'tgtRadioButton',tgtRec.type,tgtRec.name);
				count++;
			} else if (value == "-1") {
				addRow('tgtTable',i,count,'tgtRadioButton',tgtRec.type,tgtRec.name);
				count++;
...
function fillMappedTable() {
	...
	for (var i=0;i < mapDsArray.length;i++) {
		...
		addMappedRow('mappedTable',i,'chk',mapRec.type,mapRec.srcname,mapRec.tgtname);
	}
	return true;
}
