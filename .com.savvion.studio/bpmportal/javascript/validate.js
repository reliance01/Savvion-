		 ...
		 if(document.applets.length !=0 )
		 {
			      ...
			      if(fieldValid && document.applets[ name ].isXML())
			      {
				       ...
				       if(content != null)
				       {
					       ...
					       {
					                 alert("The "+label+" dataslot contains invalid xml!");
							 return false;
					       }
				       }
			      }
...
function validateDecimalField(obj, label) {
	...
	if (obj.value == "" || !isFloat(obj.value) || !isNumber(obj.value)){ 
		alert("Please enter a valid decimal value for \""+ label + "\" field!");
			return false;
	}	

	if(isNaN(parseFloat(obj.value))){
	
	 	alert(obj.value + " is not a valid decimal!! ");
		return false;
	} else {
    		...
    		var expectedScale = field[2];
   			...
   			if(fieldValue.indexOf(".") != -1) {
				if(fieldValue.substring(0, fieldValue.indexOf(".")).length > (expectedPrecision - expectedScale)) {
					alert("Maximum " + (expectedPrecision - expectedScale) + " digits are allowed before the decimal point in \""+ label + "\" field!");
					return false;
   			...
   			if(fieldValue.substring((fieldValue.indexOf(".") + 1), fieldValue.length).length > expectedScale) {
   				alert("Maximum " + expectedScale + " digits are allowed after the decimal point in \""+ label + "\" field!");
   				return false;
...
function searchUser() {
    ...
    window.open('/sbm/bpmportal/administration/UserManagementServlet?action=searchUser','SearchUser','scrollbars=yes,resizable=yes,width=900,height=600');
    return ;
}

function searchReassignUser() {

    window.open('/sbm/bpmportal/administration/UserManagementServlet?action=searchUser&queryUserByAlphabet=*&isReassign=true','SearchUser','scrollbars=yes,resizable=yes,width=900,height=600');
    return ;
}
...
function validateEmail(value,label) {
  ...
  if (value == ""){ 
	  alert("Please enter a valid email address for \""+label+"\" field!");
          return false;
  }
  ...
  if (value.indexOf(" ") >= 0) {
       ...
       alert("Email Address is wrong! Email should be in name@domain.com format.");
       return false;
...
function validateSSN(value,label) {
	...
	if (value == ""){ 
	  alert("Please enter a valid social security number for \""+label+"\" field!");
	  return false;
	}
	...
	var matchArr = value.match(/^(\d{3})-?\d{2}-?\d{4}$/);
        ...
        if (matchArr == null || numDashes == 1) {
          alert(value+' is invalid social security number. SSN must have nine digits.\nPlease enter social security number in NNN-NN-NNNN format.');
	  return false;
        }else if (parseInt(matchArr[1],10)==0) {
          alert("value+' is invalid social security number.\nSocial security number can't start with 000.");
          return false;    
       }else {
            return true;
       }
...
function validatePhoneNumber(value,label){
	...
	if (value == ""){ 
	  alert("Please enter a valid phone number for \""+label+"\" field!");
          return false;
        }
	
	if ((value.length != 0) && (value.length < 10)){
		alert("Please enter exactly 10 digits in \"" + label + "\".\n Phone number should be in ###-###-#### format!\n");
		return false;
	}
	if (value.length > 12){
		alert("Please enter exactly 10 digits in \"" + label + "\".\n Phone number should be in ###-###-#### format!\n");
	...
	for (i = 0; i < value.length; i++){
	     var ch = value.charAt(i);
	        if(((i == 3) || (i == 7)) && ((ch != '-') && (ch !=' '))){ 
		        alert(" Please enter only digits in ###-###-#### format!\n"+ch);
			return false;
	...
	if (value.length == 12){
		...
		if(value.charAt(0) < '2' ){
			alert("The first digit of the area code in \"" + label + "\" cannot be a '1' or '0'");
          	        return false;
		}else if(value.substring(0,3) == "900" ){
			alert("The area code in \"" + label + "\" cannot be '900'");
			return false;
		}else if(value.substring(0,3) == "911" ){
			alert("The area code in \"" + label + "\" cannot be '911'");
			return false;
		}else{
...
function validatePostalCode(value,label){
  ...
  if (value == ""){ 
	  alert("Please enter a valid postal code for \""+label+"\" field!");
          return false;
   }
...
function validateCreditCardDefault(value,label){
	if (value == ""){ 
	  alert("Please enter a valid credit card number for \""+label+"\" field!");
          return false;
        }
	
	if(value.length != 19){
		alert("Please enter Credit Card Number in ####-####-####-#### format.\n");
		return false; 
	}
	if( !isInteger(value.substring(0,4)) || !isInteger(value.substring(5,9)) || !isInteger(value.substring(10,14)) || !isInteger(value.substring(15,19))){
	        alert(" Not a valid Credit Card Number, Please enter a valid credit card number!.\n");
		return false;
	}	
	if( isInteger(value.substring(0,4)) && isInteger(value.substring(5,9)) && isInteger(value.substring(10,14)) && isInteger(value.substring(15,19))){
	       return true;	
...
function validateURL(value,label){
	...
	if (value == ""){ 
	  alert("Please enter a valid url for \""+label+"\" field!");
          return false;
        }
	...
	if((http == "http://") || (ftp == "ftp://") || (https == "https://"))
		return true;
	      ...
	      alert(value + " is not a valid URL.\n\n");
	      return false;
	}	
}
...
function validateCurrency(value,label) {
     if (value == ""){ 
	  alert("Please enter a valid currency value for \""+label+"\" field!");
          return false;
      }
	...
	if( (decimalPlace == -1) ){
	     alert("Currency format is incorrect!\n Currency should be in ###.## format. ");
	     return false;
        }
	else	
	return true;
...
function validateDecimal(value,label){
	...
	if(decimal.length != value.length){
		 alert(value + " is not a valid decimal! ");
	         return false;
...
function validateNumber(value,label){
	if (value == ""){ 
	  alert("Please enter a valid number for \""+label+"\" field!");
          return false;
        }
	if(isNumber(value)){
		 return true;
	}else {
	 alert("Not a valid number! Please enter a valid number for \""+label+"\" field!");
	 return false;
        }	 
}
...
function validateNumberRange(value,min,max,label){
	...
	if(isInteger(value)){
		...
		if( min <= number && number <= max){
			...
			alert("Not a valid number!"+value+" should be in "+min+" to "+max+" range for  \""+label+"\" field!");
			return false;
		}
	}else {
		alert("Not a valid number! Please enter a valid number for \""+label+"\" field!");	
...
function validateSize(nm,val, sz) {
	if(val.length > parseInt(sz)) {
		alert('Character limit exceeded for field "'+nm+'"');
		return false;
