...
public class FTPUpload { 
  ...
  /* 
   * The static block loads ftp server details, bizlogic admin user details from 
   * techdesk.properties file. techdesk.properties is located under SBM_HOME/conf 
   * folder. By using static by, this properties file will be loaded only once at 
   * ejb server startup. Any new modifications/properties will need server reboot 
  ...
  */
    ...
    static
    {
	    try
	    {
		    ...
		    //ftpUser = propertyReader.getPropertyByKey(AppConstants.RGI_FTP_USER);
		    //ftpPassWord = propertyReader.getPropertyByKey(AppConstants.RGI_FTP_PASSWORD);
		    //ftpServer = propertyReader.getPropertyByKey(AppConstants.RGI_FTP_SERVER);
		    //bl_admin_user =  propertyReader.getPropertyByKey(AppConstants.BIZLOGIC_ADMIN_USER);
		    //bl_admin_password = propertyReader.getPropertyByKey(AppConstants.BIZLOGIC_ADMIN_PASSWORD);			  
	    }
	    catch(Exception e)
	    {
	    	// do nothing
  ...
  /*
   *  getDocument() method is the main method which will get executed on ApproverFTPUpload adapter
   *  workstep. In this method document dataslot ApproverDocument/AppSupportDocument is read using 
   *  BizLogic Server APIs and then files attached to document dataslot are uploaded to ftp site.
   */  
    ...
    public void getDocument() throws Exception
    {
	    ...
	    {      
	        ...
	        for (int i=0; i < ls1.size(); i++) 
	        {
	            ...
	            doc.saveToFile(context,"/CallDesk_FTPFile_TempLoc/" + doc.getName());
	            file = new File("/CallDesk_FTPFile_TempLoc/" + doc.getName());	        
...
/*
 * upload() method uses apache commons FTPClient API and uploads files to the FTP Site
 *  
 */
  
    public void upload(String ftpServer, String user, String password, String fileName, File file) 
        throws Exception 
    {      	        
	    try 
	    { 
	        ...
	        File tempfile = new File("/CallDesk_FTPFile_TempLoc/"+fileName);             
