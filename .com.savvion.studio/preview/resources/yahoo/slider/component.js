...
jmaki.widgets.yahoo.slider.Widget = function(wargs) {
    ...
    */
    ...
    function getRealValue() {
         return Math.round(wrapper.getValue() * scaleFactor);
    }
    ...
    */
    function initSlider() {
		...
		//YAHOO.util.Dom.get(sliderThumb).title = "Valiue is "+ getRealValue();
		...
		//YAHOO.util.Dom.get(sliderThumb).title = getRealValue();
  		 var tooltip = new Ext.ToolTip({
                           target: sliderThumb,
                           title: getRealValue(),
                           plain: true,
		...
		wrapper.setValue(Math.round(wargs.value/scaleFactor));
        }
    ...
    this.setValue = function (_v) {
        var v = parseFloat(_v, 10);
	if(isNaN(v)) v = 0;
        wrapper.setValue(Math.round(v/scaleFactor));
    }
