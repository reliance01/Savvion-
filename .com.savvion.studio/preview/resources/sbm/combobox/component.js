...
jmaki.widgets.sbm.combobox.Widget = function(wargs) {
   ...
   this.getValue = function() {         	    
    ...
    this.setSelectedValue = function(selectedValue) {		
        ...
        // this logic is used for STRING DATASLOT with combo presentation
		if(selectedValue.indexOf('[') ==0){		
			try{
				var _values = YAHOO.lang.JSON.parse(selectedValue);  
				for(var i=0; i<_values.length;i++){
				    if(typeof _values[i].selected != 'undefined' && _values[i].selected ) {
					     selectedValue =  _values[i].value;
						 break;
					}
				}
			}catch(e){}
