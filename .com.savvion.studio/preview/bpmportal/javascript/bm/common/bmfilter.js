...
// their previous value or default value.
    ...
    var dsFilter = new DataSlotFilter({
        ...
        listeners: {
            afterrender: function(thisElt){
                ...
                thisElt.resizer.resizeTo(thisElt.getWidth(),thisElt.getHeight()+28);                                                   
                thisElt.resizer.el.setStyle('padding-bottom', '15px');
            ...
            */
            afterinsertrow : function(dsFilter) {
                /*
                    once a row is inserted, we add 50 pixels to the container height.
                    This will increase the dsFilter height and will make the user
                    aware that a now row is added
                */
                dsFilter.resizer.resizeTo(dsFilter.getWidth(),dsFilter.getHeight()+50);
            },
            /*
            ...
            */
            afterdeleterow : function(dsFilter) {
                /*
                    once a row is deleted, we subtract 5 pixels from the container height.
                    we are not subtracting 50px here as we did during insert, 
                    because the resize bar need not be resized here.
                */
                dsFilter.resizer.resizeTo(dsFilter.getWidth(),dsFilter.getHeight()-5);
            }
        },
    ...
    this.dataslotPanel = new Ext.Panel({
        id : 'dsFilterEnclPanel',
        layout : 'form',
        hideBorders:true,
        frame : false,
        margins : '0 0 0 0',
        title   : '<span class="BmFilterHiliteTxt">'+getLocalizedString('advanced')+'</span>',           
        headerCfg:{
            tag : 'div',
    ...
    var prepareButtons = function(config) {
        ...
        buttonsArray.push({
            ...
            id : 'searchBtn',
            //isQuickSearch : config.isQuickSearch,
            handler : function() {
                //if(!this.isQuickSearch && !Bm.view.validateDsFilter()) {                                
    ...
    this.render = function(config){
        ...
        /**if(!config.isQuickSearch){  
            ...
            this.tabSearchPanel = [this.queryPanel];
...
function resetForm(form){
   resetDateFields();
     ...
     var qCmb = getQueueCombo();
     qCmb.setDisabled(true);
...
var formValues = prepareParams();    
    ...
    if(store.getSortState()){
        ...
        options.params['dir'] = store.getSortState().direction;
    }else{
...
function getAppFilterCfg(values){
    var cfg = new Bm.filter.Panel({items : [{xtype:'bmappfilter',
        id : 'app',
...
function getVersionFilterCfg(values){
    var cfg =  new Bm.filter.Panel({items : [{                  
...
function getStatusFilterCfg(values){
    var handleDsPanel = function(statusCmbValue){
        if('collaborativeassigned' == statusCmbValue 
                || 'delegatedavailable' == statusCmbValue
                || 'delegatedassigned' == statusCmbValue) {
            disableDsFilterPanel();
        }
    };
    var cfg = new Bm.filter.Panel({items : [{
...
function getAppStore(values){
    ...
    var appStore = new Ext.data.JsonStore({        
        fields: [
            'value',
            'valueLabel'
        ],                       
...
function getPerformerTypeStore(values){
    ...
    var performerTypeStore = new Ext.data.ArrayStore({
        fields: ['value', 'valueLabel'],
        data : data
    });
...
function getWsAndDsStore(selectedWorkstep){
    var workstepStore = new Ext.data.SimpleStore({        
        proxy: new Ext.data.HttpProxy({url: '../myhome/bmTaskSearch.portal',method:'GET'})           
...
function getDataSlotStore(wsStore){
    ...
    if(values == undefined || values[0] == undefined)
        return; 
    var dsValues = values[0].json.dataslotList;      
        var dataSlotStore = new Ext.data.JsonStore({        
        fields: [
                 'type',
                 'name',
                 'valueLabel',                 
...
function getWsStore(store){
    ...
    wsValues = wsValues.concat(values[0].json.workstep);
    
    var wsStore = new Ext.data.SimpleStore({        
        fields: [
                    'value',
                    'template',
                    'valueLabel'
...
function getQueueStore(values){ 
  ...
  var queueStore  = new Ext.data.JsonStore({
        fields: [
            'value',
            'valueLabel'
        ],                         
...
function getStartDateRangeStore(values){    
    ...
    var stDateRangeStore  = new Ext.data.JsonStore({
        fields: [
            'value',
            'valueLabel'
        ],                         
...
function getDueDateRangeStore(values){  
   ...
   var dueDateRangeValues = values;
    ...
    var dueDateRangeStore  = new Ext.data.JsonStore({
        fields: [
            'value',
            'valueLabel'
        ],                         
...
function getFilterStore(appName,versionName,values){
  ...
  var filterValues = [{id : '',displayName : getLocalizedString('filter.combobox.no.filter')},{id : '',displayName : globalFilterStr}];
    var globalFilterData = values[0].json.filters.globalFilters;
    var taskFilterData = values[0].json.filters.taskFilters;
