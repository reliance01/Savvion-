...
function validateForm()
{
	...
	if(startTime >= endTime) {
		alert(calenderi18["bizCalAlert54"] );
		document.forms[0].splBizStartTimeHrs.focus();
		return (false);
	}
    return true;
...
function saveWorkingHrs(val)
{
    ...
    if(tmslot1 != "00:00-00:00")
    {
        ...
        if(opt == 'date') {
            if(opener.isNonWorkingTimeExist(date)) {                
                alert(calenderi18["bizCalAlert64"]);
                return false;
            }
        }
        ...
        {
		if(!confirm(calenderi18["bizCalAlert55"]))
			return false;
        }
        opener.updateWorkingTable(day,date,time,val);
        opener.disableBtn();
    }
    return true;
}
   ...
   if(day != "-")
    {
        ...
        {
            if(day == theForm.bizHourDOW.options[i].value)
            {
    ...
    if(date != "-")
    {
        ...
        {
            if(theForm.bizHourMonth.options[i].value.indexOf(mn) != -1)
            {
