...
/**
  * Ext.ux.form.field.ClearButtonX Extension Class
  *
  *
  * @class Ext.ux.form.field.ClearButtonX
  * @extends Ext.ux.form.field.ClearButton
  * 
  * This wrapper class is used in portal pages, this needs to be upgraded while upgrading clearbutton plug-in.
  * 
  */
  
Ext.define('Ext.ux.form.field.ClearButtonX',{
    extend : 'Ext.ux.form.field.ClearButton',
    alias : ['plugin.clearbuttonx'],
    
    /**
     * If the decimal field is readonly. 
     * This will not show the clearbutton button and will not clear the value on pressing Esc key.
     * After the field has been rendered sets up the plugin (create the Element for the clear button, attach listeners).
     * @private
     */
    handleAfterRender: function(textField) {
    ...
    /**
     * This will left align the clear button and avoid the white space appear on right side while using the spinner of decimal field. 
     * Called after any event that may influence the clear button visibility.
     */
    updateClearButtonVisibility: function() {
        var oldVisible = this.isButtonCurrentlyVisible();
        ...
        if (oldVisible != newVisible) {
            ...
            if (!(this.isTextArea && Ext.isGecko) && !Ext.isIE) {
                // See https://bugzilla.mozilla.org/show_bug.cgi?id=157846
                var deltaPadding = clearButtonEl.getWidth() - this.clearButtonEl.getMargin('l');
                if (this.textField.clearButtonAlign == 'left') {       
                    var deltaPadding = this.clearButtonEl.getMargin('l');
                }
                var currentPaddingRight = this.textField.inputEl.getPadding('r');
                var factor = (newVisible ? +1 : -1);
                this.textField.inputEl.dom.style.paddingRight = (currentPaddingRight + factor * deltaPadding) + 'px';
            }
        }
        if (this.textField.clearButtonAlign == 'left') {       
            clearButtonEl.alignTo(this.textField.bodyEl, 'tl-tl', [3,5]);
        } else {
            /* repositionClearButton is a private function of the clear button plug-in
             * If the textfield is reposition after render, the clearbutton will appear on the previos position[fvalidate error message will reposition the text field].
             * To avoid this behaviour, the clear button is repositioned.
            */
            this.repositionClearButton();
        }
    }
