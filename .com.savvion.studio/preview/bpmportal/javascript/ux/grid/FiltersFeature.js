...
Ext.define('Ext.ux.grid.FiltersFeature', {
     ...
     */
    menuFilterText : getLocalizedString('ColumnFilter'),
    /**
    ...
    updateColumnHeadings : function () {
        ...
        if (headerCt) {
            headerCt.items.each(function(header) {
                ...
                //Enable 'Clear Grid Filters' Button                
                var clrFiltersBtn = Ext.getCmp('clrFiltersBtn');
                if(filter && filter.active && !Ext.isEmpty(clrFiltersBtn)) {
                    clrFiltersBtn.enable();                    
                }
            });
        }
    },
    ...
    clearFilters : function () {
        ...
        //Disable 'Clear Grid Filters' Button                
        var clrFiltersBtn = Ext.getCmp('clrFiltersBtn');
        if(!Ext.isEmpty(clrFiltersBtn)) {            
            clrFiltersBtn.setDisabled(true);
        }
    },
    ...
    /**
     * Returns an String of the currently active filters.
     * @return {String} filters string of the currently active filters.
     */
    getFilterDataAsString : function (ColumnModel) {
        var filtersStr = "", i, len;
        this.filters.each(function (f) {
            if (f.active) {
                var d = [].concat(f.serialize());
                len = d.length;
                for (i = 0; i < len; i++) {
                    var dataIndex = f.dataIndex;
                    var header = dataIndex;
                    for (j=0; j< ColumnModel.length; j++) {
                        if (dataIndex == ColumnModel[j].dataIndex) {
                            header = ColumnModel[j].text;
                        }
                    }
                    filtersStr += header + " = " + d[i].value + "<BR/>";
                }
            }
        });
        return filtersStr;
