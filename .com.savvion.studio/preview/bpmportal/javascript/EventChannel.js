...
function validate(isAdd)
{
    ...
    if (isAdd)
    {
        if ( getElementValue( "channelName" ) == '' ||
            ...
            alert(getLocalizedString('EventChannelAlert2'));
            return false;
        }
    }
    if (matchPasswords())
...
function matchPasswords()
{
    ...
    {
        alert(getLocalizedString('EventChannelAlert3'));
        return false;
    }
    return true;
}
...
function submitContextData(isAdd) {
   // send data to parent
    ...
    if (isAdd)
    {
        ...
        {
            alert(getLocalizedString('EventChannelAlert4'));
            return false;
        }
        if (ctxName.substring(0,4) != "CTX.")
        {
              alert(getLocalizedString('EventChannelAlert5'));
              return false;
        }
        if (ctxName.indexOf(" ") != -1)
        {
             alert(getLocalizedString('EventChannelAlert6'));
             return false;
        }
    }
    else
    {
        ...
        {
            alert(getLocalizedString('EventChannelAlert7'));
            return false;
        }
    }

    if ( ctxValue == '' || ctxValue == null ) {
        alert(getLocalizedString('EventChannelAlert8'));
        return false;
    }
...
function saveAndAddContextData(isAdd)
{
    // send data to parent
        ...
        if (isAdd)
        {
            if ( ctxName == '' || ctxName == null )
            {
                alert(getLocalizedString('EventChannelAlert9'));
                return false;
            }
            if (ctxName.substring(0,4) != "CTX.")
            {
        ...
        else
        {
            ...
            if ( ctxName != opener.document.forms[0].ctxConditionName.value )
            {
                alert(getLocalizedString('EventChannelAlert10'));
                return false;
            }
        }

        if ( ctxValue == '' || ctxValue == null ) {
            alert(getLocalizedString('EventChannelAlert11'));
            return false;
        }
...
function addContextData() {
   ...
   //FIXME: dolayout
   var channelFilterConditionTab = Ext.getCmp('channelFilterConditionTabId');
   if (!Ext.isEmpty(channelFilterConditionTab)) {
        channelFilterConditionTab.doLayout();
   }
}
