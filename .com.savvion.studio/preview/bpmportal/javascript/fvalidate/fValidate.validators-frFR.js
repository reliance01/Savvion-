...
fValidate.prototype.number = function( type, lb, ub )
{
     ...
     //the following additional processing performed for french ...
     
	if(type != 0) { //for decimal values...
		var fmt = /^(-?\d+,\d+)$/;
		if (fmt.test(elementValue)) { //if it is in valid french format, then we replace the comma with the dot and uses english version validation.
