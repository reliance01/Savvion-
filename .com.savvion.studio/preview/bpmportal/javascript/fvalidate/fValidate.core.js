...
function fValidate( f, errorMode, groupError )
{
    ...
    function addLabelProperties()
    {
        ...
        while ( label = labels[i++] )
        {
	        ...
	        // added by keivan
            //alert("$(label.htmlFor).getAttribute('alt'):"+$(label.htmlFor).getAttribute('alt'));
	        //alert("f.elements[label.htmlFor]:"+f.elements[label.htmlFor]);
	        //if(label.htmlFor.startsWith('bizsite_docSlotButton_')){
	    
	        if(typeof f.elements[label.htmlFor] != 'undefined' ){		    
		       if(f.elements[label.htmlFor].getAttribute){
			       var fName = f.elements[label.htmlFor].getAttribute('name');			       
				   if(fName == null){
				   	  continue;					
				   }
				   if(fName.substr(0,22) == 'bizsite_docSlotButton_'){		       
			           elem = f.elements["bizsite_dataslot_"+label.htmlFor];
		               elem.label = label;
                       continue;
		           }
			   }
	        ...
	        if(typeof f.elements[label.htmlFor] == 'undefined' )
            {                
		        if(typeof f.elements["bizsite_dataslot_"+label.htmlFor] != 'undefined' )
                {
                          elem = f.elements["bizsite_dataslot_"+label.htmlFor];
			              if($("bizsite_dataslot_"+label.htmlFor) != null)
                          {
                              if(Ext.isIE)
                              {
                                  elem = $("bizsite_dataslot_"+label.htmlFor);
                                  if((typeof $("bizsite_dataslot_"+label.htmlFor)["alt"] == 'undefined') || ($("bizsite_dataslot_"+label.htmlFor)["alt"] == "")) 
                                  {
                                      continue;
                                  }
                                  else if(($("bizsite_dataslot_"+label.htmlFor)["alt"] ==  "document") || ($("bizsite_dataslot_"+label.htmlFor)["alt"] == "xml"))
                                  {
                                      elem.label = label;
                                      continue;
                                  }
                              ...
                              else
                              {
                                  ...
                                  else if(($("bizsite_dataslot_"+label.htmlFor).getAttribute("alt") ==  "document") || ($("bizsite_dataslot_"+label.htmlFor).getAttribute("alt") == "xml"))
                                  {
                                      elem.label = label;
                                      continue;
                                  }
            ...
            else if ( typeof elem.length != 'undefined' && elem.length > 1 && elem.nodeName != 'SELECT' && elem.nodeName != undefined)
...
fValidate.prototype.throwError = function( args, which )
{
    ...
    {   if(typeof this.skip == 'undefined' || this.skip){
	//  skip        
        elem.validated = true;
        return;
        }
...
{
    //alert("-----------------------------------------showError");
    //alert("elem.name:"+this.elem.name+
    //"\nelem.label:"+this.elem.label);
    ...
    if((isHidden) || (typeof this.elem.label != 'undefined'))
        label = this.elem.label;
    // fix for radio the label does not acquire error css
	if(label == null){
		label = this.findLabel(this.elem);		
	}
		
    if ( typeof this.showErrors == 'undefined' ) this.showErrors = new Array(); 
    
    //alert(this.errorMode);
    ...
    {   //  This represents all possible combinations
        ...
        case 28 : inputError(); labelError(); boxError(); alertError(); break;      
    }
    //  Regular alert error
    function alertError()
    {
    ...
    {
        //added by keivan
        //remove input operation for input hidden..
        if(isHidden)return;
    ...
    function boxError()
    {
        ...
        {
            errorElem.innerHTML = emsg.toHTML();
        }
        else
        {
            ...
            errorElem.onclick = function()
            {
                ...
                if ( typeof elem.length != 'undefined' ) elem = elem[0];
				if( elem.type != "hidden" && elem.style.display != "none"  && !elem.disabled ) {  
    	      		  elem.focus();  
    	      		  return;  
	      	      }
            }
...
fValidate.prototype.revertError = function( elem )
{
    ...
    if ( ( typeof elem.length != 'undefined' && elem.length > 1 && elem.nodeName != 'SELECT' ) || isHidden )
    {
	    ...
	    // added for fixing the revert issue for radio control
		try {
			var _label = this.findLabel(elem[0]);
			this.removeCSSClass(_label, errorClass);
		}catch(e){}
	} else {
...
fValidate.prototype.selectFocus = function( elem )
{
    ...
    if ( typeof elem.select != 'undefined' ) elem.select();
    if ( !ie && typeof elem.focus != 'undefined' ) { try{elem.focus();}catch(e){}}
...
------------------------------------------- */
if ( typeof Array.prototype.push == 'undefined' && typeof Array.push == 'undefined')
