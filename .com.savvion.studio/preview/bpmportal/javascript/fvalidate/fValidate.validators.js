...
fValidate.prototype.number = function( type, lb, ub )
{
    ...
    if(type==0 && !fail && this.elem != undefined) 
        fail = Boolean(this.elem.value.indexOf(".") != -1 );    
    if ( !fail )
    {
        switch( true )
        {
...
fValidate.prototype.length = function( len, maxLen )
{
    ...
    //if element type is textarea and maxLen validation is present then take storage limit as maxlen to  fix bug sbm-2652
        var elemType = this.elem.type;
        if(elemType == 'textarea' && (maxLen == null || maxLen == undefined || maxLen == '')){
               maxLen = this.elem.getAttribute("maxlength");       
    }
...
/*/>*/
/*< url web *******************************************************************/
fValidate.prototype.url = function( hosts, hostOptional, allowQSFlag )
{
    if ( this.typeMismatch( 'text' ) ) return;
    ...
    //regular expression modified as case insensitive to fix RPM00027137
...
fValidate.prototype.ip = function( portMin, portMax )
{
    ...
    {
       var dsprefix = "bizsite dataslot";
       if(typeof this.elem.label == 'undefined') {
            if(fieldName.indexOf(dsprefix) != -1) {
                this.throwError( [fieldName.substr(dsprefix.length,fieldName.length)] );
            }
            else {
                this.throwError( [fieldName] );
            }
    ...
    else
    {
        ...
        while ( part = parts[i++] )
        {
            if ( i == 5 ) // Check port
            {
                if ( Number(part) < Number(portMin) || Number(part) > Number(portMax) )
                {
...
fValidate.prototype.checkbox = function( minC, maxC )
{
    ...
    {
        this.throwError( [this.elem.fName],2);
    }
    else
    {
        ...
        {
            var fieldName = this.elem[0].fName;
            if(fieldName != undefined && fieldName.indexOf("bizsite dataslot") > -1) {
               fieldName = fieldName.substr(16,fieldName.length)
            }
            var parent = this.elem[0].parentNode.parentNode;
            if(parent == 'undefined') {
                this.throwError( [this.elem[0].fName] );
            } else {
                var fieldLabel = this.getFieldLabel(this.elem[0], parent);
                if(fieldLabel == null || fieldLabel == 'undefined') {
            this.throwError( [minC, maxC, fieldName, count] );
                } else {
                    this.throwError( [minC, maxC, fieldLabel, count] );
                }
            }
        }           
    }
...
fValidate.prototype.radio = function()
{
    ...
    {   
		...
		var _label = this.findFieldLabel(this.elem[0]);	
        this.throwError( [_label] );       
    }
}
fValidate.prototype.findFieldLabel = function(elem) 
{
    var id = elem.id;
    var form = document.forms[0];
    var labels = form.getElementsByTagName("LABEL");
    fieldLabel = elem.fName;
    
    for(i=0;i<labels.length;i++) {  	
        if(typeof labels[i].htmlFor != 'undefined' && labels[i].htmlFor == id) {            
	    fieldLabel = labels[i].innerHTML;
            break;
        }            
    }    
    return fieldLabel;
...
fValidate.prototype.findLabel = function(elem) 
{
    var id = elem.id;
    var form = document.forms[0];
    var labels = form.getElementsByTagName("LABEL");
...
fValidate.prototype.getFieldLabel = function(elem,parent) 
{
    var id = elem.id;
    var values = parent.getElementsByTagName("font");
    var fieldLabel = null;

    //Following code is added to get font tag in case of auto generated presentation
    if(values.length == 0){
        var mapLabel = "LabelDIV_"+elem.name;
        var formLabelElem = document.getElementById(mapLabel);
        if(formLabelElem != null && formLabelElem != 'undefined') {
            values = formLabelElem.getElementsByTagName("font");
        }
    }

    for(i=0;i<values.length;i++) {
        var node = values[i].getElementsByTagName("label");
        if(node.length > 0 && node[0].getAttribute("for") == id) {
            fieldLabel = sbm.filterLabel(node[0].childNodes[0].nodeValue);
            break;
        }            
    }
    return fieldLabel;
