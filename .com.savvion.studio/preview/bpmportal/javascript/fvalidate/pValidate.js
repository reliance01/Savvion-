...
sbm.pValidate = function(formElem,vType)
{
     ...
     //--------------------------------------------------------------------------------------------------------///
     var createBtn = document.forms[ 0 ].elements['bizsite_createInstance'];
     if(createBtn != null && createBtn != "undefined"){
        createBtn.disabled = true;
     }
     var completeBtn = document.forms[ 0 ].elements['bizsite_completeTask'];
...
{   
    ...
    while ( elem = f.elements[i++] )
    {
            ...
            if(elem.name == 'bizsite_instanceName'){
                  var instanceName = elem.value;
                  var invChars = "!@#%*:$'\"?\/>\\\\<";
                  var regExp_invChars=new RegExp("["+invChars+"]");
              if(!sbm.isValidStr(instanceName, regExp_invChars)){
                  alert(f.fv.i18n.errors.bizsite_instanceName[0]+invChars);
                  return false;
              }
            }
            if ( !( typeof fvCode == 'undefined' || fvCode == null || fvCode == "" ) )
            {
                ...
                if(clickedButton.toLowerCase() == "bizsite_savetask" || clickedButton.toLowerCase() == "bizsite_completetask"){                    
                    if(elem.tagName == 'TEXTAREA'){                        
                        if(/^[0-9]+$/.test(elem.getAttribute("maxlength"))) {
                            
                            var len = parseInt(elem.getAttribute("maxlength"), 10);
                            var currLen = elem.value.replace(/(\r\n|\n|\r)/g, "11").length;//FF considers \n as 1 character.                            
							var errArgs = [elem.getAttribute("name").substring(17), len, currLen];
                            if(currLen > len) {                                                               
                                f.fv.throwError(errArgs, 1);                                
                            }                           
                        }                                           
                    }                    
                }
            ...
            var cmp = Ext.getCmp(elem.name);
            if(!Ext.isEmpty(cmp) && cmp.xtype == 'decimaldatafield') {  //Validation will be done only to Ext Component for Decimal Dataslot
                validateDecimalDataField(elem, f.fv);
            }
     } //   end of element loop
    ...
    function clearErrorMsg( elem ) {
        ...
        if ( typeof f.fv.boxError != 'undefined' ) {
            if ( typeof f.fv.boxError.normalize != 'undefined' ) f.fv.boxError.normalize();
            if ( errorElem = document.getElementById( f.fv.config.boxErrorPrefix + elem.name ) ) {                
                f.fv.boxError.removeChild( errorElem );
            }
            if ( f.fv.boxError.childNodes.length == 0 ) f.fv.boxError.style.display = "none";
        }
    }
}
//---------------------------------------------------------------------------------------------------------//
...
fValidate.prototype.document = function()
{
    ...
    if((document.applets != undefined) && (document.applets.length !=0 ) && (document.applets[ this.elem.name ] != undefined))
    {
    ...
    else if((document.embeds != undefined) && (document.embeds.length !=0 ) && (document.embeds[ this.elem.name ] != undefined))
    {
...
{
     if((typeof this.elem != 'undefined') && this.elem.bok == true){
        if(this.elem.value == ''){
            this.elem.validated == true;
            return;
        }
     }
     if(!Date.isSBMDateValid(this.elem.value,format))
     {
         //this.throwError( [this.elem.fName,format] );
         format = format.replace(/ a+/g," "+getLocalizedString("am")+"/"+getLocalizedString("pm"));
         if(typeof this.elem.label == 'undefined')
         {
              var fieldName = this.elem.fName;
              if(fieldName.indexOf("bizsite dataslot ") != -1)
...
fValidate.prototype.xml = function()
{
    ...
    /*if((this.elem.name).indexOf("bizsite_dataslot_")>-1) {
        this.elem.name = (this.elem.name).substr("bizsite_dataslot_".length,this.elem.name.length);
    }*/
...
sbm.isValidStr = function ( name, invalidChars ) {    
...
validateDecimalDataField = function (elem, ffv) {
    var cmp = Ext.getCmp(elem.name);
    ffv.elem = elem;
    ffv.type = 'datafield_decimal';
    if(cmp.hasActiveError()) {
        var msg = cmp.getActiveError();
        msg = msg.replace(/<br>/g, ' ');
        ffv.throwError([msg],0);
    } else {
        elem.validated = true;
    }
