...
{
    MM_goToURL('self','../administration/mig_instance_list.jsp');
}
...
{
    MM_goToURL('self','../administration/favorites_list.jsp');
}
...
function goToBLApps()
{
    MM_goToURL('self','../administration/SBMAdminAppServlet?action=bizlogicAll');
}
function goToBSApps()
{
...
{
    MM_goToURL('self','../myhome/create_collaboration.jsp');
}
...
function deployAppPack() {
    var appPackPopupWnd = new Ext.Window({
        ...
        buttons:[{
            ...
            handler : function() {
                ...
                Ext.Ajax.request({
                    url: getLocalizedString('ctxp') + '/bpmportal/ajaxutil.portal?action=exportAppPack',
                    method : 'post',
    ...
    if(!Ext.isIE) {
       appPackPopupWnd.setSize({width:500,height:135}); 
    } else {
       appPackPopupWnd.setSize({width:500,height:150}); 
    }
