  ...
  // Adds the stored multiselect control values - e.g. is used for String dataslot selection from checkbox
  function addMultiSelectValues(name, selections) {  	  
	  for(var j=0; j < values.length; j++) 
	  {
	   	   if(name == values[j][0])
	   	   {
	  ...
	  for(var i =0; i<_elements.length;i++){
	       if(name == _elements[i].name){		        
		        var el = _elements[i];
				if(el.type == 'checkbox'){
				      el.addEventListener('change', function(o){					       
