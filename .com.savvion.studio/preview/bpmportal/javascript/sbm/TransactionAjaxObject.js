if(!sbm) sbm = {};
sbm.utils = sbm.utils || {};
YAHOO.namespace ("YAHOO.portal");
YAHOO.namespace ("YAHOO.portal.container");

var transactionAjaxObject = { 
	    start:function(type, args){ 
		// start   
	        // transaction event: 'args[0].tId' type: type has been fired.
		//if(!YAHOO.lang.isUndefined(args[0]) && !YAHOO.lang.isUndefined(args[0].argument[1]) 
		//	    && YAHOO.lang.isBoolean(args[0].argument[1]) && (args[0].argument[1] == true))
	    ...
	    failure:function(type, args){ 
                ...
                if(args[0].responseText != undefined){ 
				   ...
				   //alert( YAHOO.lang.JSON.stringify(args[0].responseText)); 
				   //alert(YAHOO.lang.JSON.stringify(data["exceptionLocalizedMessage"]));
				   var win;
				   var content = "<span style='color: red'>"+YAHOO.lang.JSON.stringify(data["exceptionLocalizedMessage"])+"</span>";
                   if(data["exceptionStackTrace"] != undefined)
                       content += ("<span style='color: black;'>"+ data["exceptionStackTrace"] + "</span>");
...
sbm.utils.setContent = function(div,content){
	...
	var matches = content.match(new RegExp(scriptExpr, "img"));
		...
		for (var i = 0; i < numMatches; i++)
		{
			...
			oScript.text = s;
			YAHOO.util.Event.onDOMReady(function(){if(oScript.text.length > 0)div.appendChild(oScript);});	
		}
	}   
}
