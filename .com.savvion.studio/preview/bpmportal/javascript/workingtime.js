...
function refreshWorkingTable(tableName) {
   ...
   //FIXME: dolayout
   var calendarWorkingTimeTab = Ext.getCmp('calendarWorkingTimeTabId');
   if (!Ext.isEmpty(calendarWorkingTimeTab)) {
		calendarWorkingTimeTab.doLayout();
   }
   return true;
}
...
function addRowToTable(tableName,day,date,time,srno)
{
  ...
  if(day == "-") {
    ...
    newCell.innerHTML="<A href=javascript:popworkingwindow('day',"+srno+") class=TblLnk>"+getLocalizedString(day)+'...'+"</a>";
...
function updateWorkingTable(day,date,time,val)
{
    ...
    for(var count=0;count < workingHrsArray.length;count++)
    {
        ...
        if(count == val || (wday == day && wdate == date))
        {
            found = true;
            idx=count;
        }
...
function deleteWorkingTime()
{
  ...
  if(isConfirm == false){
	   alert(calenderi18["bizCalAlert57"]);
	   return false;
  }
  if (isConfirm == true){
  if (confirm(calenderi18["bizCalAlert58"])) {
...
function isNonWorkingTimeExist(date) {
    //Check user selected date exists in Non-working date
    var isDateExist = false;
    var tempDate;
    for(var ix=0;ix < nonworkingHrsArray.length;ix++) {
        var yr = nonworkingHrsArray[ix].year;
        var mn = nonworkingHrsArray[ix].month;
        var dt = nonworkingHrsArray[ix].date;        
        tempDate = mn.substring(0,3) + " "+ dt +", "+yr;
        if(date == tempDate) {
            isDateExist = true;
            break;
        }
