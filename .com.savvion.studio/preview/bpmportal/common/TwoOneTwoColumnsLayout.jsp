...
<td class="layout-column-spacer" width="">
<div>&nbsp;</div>
</td>
<td valign="top" width="50%">
...
<td valign="top" width="50%">
<div class="" id="layout-column_column-5">
<c:if test="${not empty columnFive}">
	<c:forEach var="widget" items="${columnFive}" varStatus="status">
		<%@ include file="../common/widget.jsp" %>
	</c:forEach>
</c:if>
<div class="layout-blank-portlet"></div></div>
...
<div id="layout-bottom-container"></div>
 ...
 // <![CDATA[
   function reorderAfterDelete(id) {
   	...
   	reorderLayout('changelayout.portal',document.DashboardCreate.layoutSelection,Sortable.serialize("layout-column_column-5",{name:'columnfive'})); 
   ...
   Sortable.create("layout-column_column-1",
     {dropOnEmpty:true,tag:'div',handle:'portlet-header-bar',hoverclass:'portlet-dragging-placeholder',containment:["layout-column_column-1","layout-column_column-2","layout-column_column-3","layout-column_column-4","layout-column_column-5"],constraint:false,
     	onUpdate:function(){
     	   reorderLayout('changelayout.portal',document.DashboardCreate.layoutSelection,Sortable.serialize("layout-column_column-1",{name:'columnOne'}));
      	}
     });
   ...
   Sortable.create("layout-column_column-5",
     {dropOnEmpty:true,tag:'div',handle:'portlet-header-bar',hoverclass:'portlet-dragging-placeholder',containment:["layout-column_column-1","layout-column_column-2","layout-column_column-3","layout-column_column-4","layout-column_column-5"],constraint:false,
     	onUpdate:function(){
     	   reorderLayout('changelayout.portal',document.DashboardCreate.layoutSelection,Sortable.serialize("layout-column_column-5",{name:'columnFive'}));
      	}
