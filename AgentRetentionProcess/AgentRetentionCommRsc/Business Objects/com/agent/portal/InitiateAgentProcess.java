...
public class InitiateAgentProcess {
	...
	public void createProcessInstance(HashMap dataslots) {
		try {
			...
			ProcessInstance pi = blserver.createProcessInstance(blsession,
					ptname, hmAttributes, dataslots, true);
			...
			System.out.println("Instance created sucess fully!!!" + id
					+ " AgentCode is " + agentCode + " role is " + role);
			updateProcessInstanceFlag(processInstanceId,agentCode,role);
	...
	public void getInstanceCreationData() {
		...
		try {
			...
			if (rset != null) {
				...
				while (rset.next()) {
					HashMap<String, String> dataslots = new HashMap<String, String>();
					...
					createProcessInstance(dataslots);

				}
				
				updateNonCreationFlag();
			}

		} catch (Exception e) {
			System.out.println("Error while getting insatance creation data : "
					+ e.getMessage());
		} finally {
	...
	public void updateProcessInstanceFlag(String processInstanceId,
			String agentCode, String role) {
		Connection connection = null;
		CallableStatement cstmt = null;
		...
		if (processInstanceId != null && agentCode != null) {
			...
			try {
			      String query = "{ call AGENT_RETENTION_UTILITY(?,?,?,?,?)}";
					...
					System.out
							.println("Error occured while updating instance cretion flag-- Method-updateProcessInstanceFlag : "
									+ e.getMessage());
				} finally {
					try {
						if (connection != null) {
	...
	public void updateNonCreationFlag()
	{
		...
		try {
		      String query = "{ call AGENT_RETENTION_UTILITY(?,?,?,?,?)}";
				...
				System.out
						.println("Error occured while updating non creation flag-- Method Name- updateNonCreationFlag : "
								+ e.getMessage());
			} finally {
				try {
					if (connection != null) {
