package rgicl.ecsehc.savvion.FinanceMasterFlow;
...
public class FinanceMaster {
   ...
   public FinanceMaster(String propertiesPath) {
   }

   public ResponseObject FinanceMasterFlowTask(RequestObject[] reqObj) {
      String methodName = "UpdateAndCompleteFinanceMasterFlowTask";
            ...
            try {
               ...
               if (processInstanceName != null && !processInstanceName.equals("") && workItemName != null && !workItemName.equals("") && !uc.checkNull(workItemName)) {
                  if (userId != null && !userId.equals("")) {
                     ...
                     if (workItemName != null && workItemName.contains("FinanceMasterMaker")) {
                        hm.put("isFinanceMasterMakerAccepted", workItemCaseStatus);
                     }

                     if (workItemName != null && workItemName.contains("FinanceMasterChecker")) {
                        hm.put("isFinanceMasterCheckerAccepted", workItemCaseStatus);
                     }

                     if (!uc.checkNull(hashMap.get("TRANSACTIONID"))) {
                        hm.put("TransactionID", hashMap.get("TRANSACTIONID").toString());
                     }
               ...
               if (var35.getMessage().contains("Incorrect userName/password for")) {
                  try {
                     ...
                     pass = p.encrypt(pass);
                     automatictasklog.info("ECS Savvion FinanceMaster " + methodName + " userId " + userid + " encrypted " + pass);
                  } catch (Exception var33) {
                     automatictasklog.error("ECS Savvion FinanceMaster: " + methodName + " :: Exception " + var33);
                  }
               }
            }
         } finally {
