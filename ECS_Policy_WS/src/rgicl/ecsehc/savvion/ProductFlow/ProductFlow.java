package rgicl.ecsehc.savvion.ProductFlow;
...
public class ProductFlow {
   ...
   public ProductFlow(String propertiesPath) {
   }
            ...
            try {
               ...
               if (processInstanceName != null && !processInstanceName.equals("") && workItemName != null && !workItemName.equals("") && !uc.checkNull(workItemName)) {
                  if (userId != null && !userId.equals("")) {
                     ...
                     if (!uc.checkNull(hashMap.get("PRODUCTNAME"))) {
                        hm.put("ProductName", hashMap.get("PRODUCTNAME").toString());
                     }

                     if (!uc.checkNull(hashMap.get("PRODUCTCODE"))) {
                        hm.put("ProductCode", hashMap.get("PRODUCTCODE").toString());
                     }

                     if (!uc.checkNull(hashMap.get("PRODUCTCOVERTYPE"))) {
                        hm.put("ProductCoverType", hashMap.get("PRODUCTCOVERTYPE").toString());
                     }

                     if (!uc.checkNull(hashMap.get("SERVICECATEGORY"))) {
               ...
               if (var35.getMessage().contains("Incorrect userName/password for")) {
                  try {
                     ...
                     automatictasklog.error("ECS Savvion :: " + methodName + " :: Exception " + var33);
                  }
               }
            }
         } finally {
